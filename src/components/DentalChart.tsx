  ...
  return (
    <motion.button
      ...
      className={`relative w-12 h-12 rounded-lg border-2 flex items-center justify-center transition-all duration-200 ${
        isInvalid
          ? 'bg-red-50 border-red-500 shadow-lg shadow-red-100 ring-4 ring-red-500/10'
          : isActive
          ? 'bg-white border-blue-600 shadow-lg shadow-blue-50 ring-4 ring-blue-600/10'
          : isSelected
    ...
    >
      <span className={`text-xs font-black select-none ${
        isInvalid ? 'text-red-600' : isActive ? 'text-blue-600' : isSelected ? 'text-blue-400' : 'text-gray-400'
      }`}>
        {number}
      </span>
      {hasProsthesis && (
        <div className={`absolute top-1 right-1 w-2 h-2 rounded-full border border-white ${
          isInvalid ? 'bg-red-600' : isActive ? 'bg-blue-600' : 'bg-blue-400'
        }`} />
      )}
    </motion.button>
  );
...
export const DentalChart: React.FC<DentalChartProps> = ({ 
  ...
  return (
    <div className="space-y-8 p-6 bg-gray-50/50 rounded-3xl border border-gray-100">
      {/* Upper Jaw */}
      <div className="space-y-3">
        <div className="flex justify-between items-center px-1">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('upperJaw')}</span>
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center px-1">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('lowerJaw')}</span>
          <div className="h-px flex-1 mx-4 bg-gray-100"></div>
        </div>
        <div className="grid grid-cols-8 sm:grid-cols-16 gap-2">
          {lowerTeeth.map((num) => (
