...
import { ChevronRight, Plus, Info, AlertTriangle, X, ShoppingCart, Check, HelpCircle, AlertCircle, ShieldAlert, FileText, Activity, Clock, Download, Trash2, Send, CheckCircle, User, Edit, Bot } from 'lucide-react';
...
export default function App() {
  ...
  const [chatType, setChatType] = useState<'bot' | 'admin'>('bot');
  ...
  useEffect(() => {
    if (chatHistory.length === 1 && chatHistory[0].role === 'bot') {
      setChatHistory([{ role: 'bot', text: chatType === 'admin' ? t('adminInitialMessage') : t('chatInitialMessage') }]);
    }
  }, [language, chatType]);

  const handleOpenChat = (type: 'bot' | 'admin' = 'bot') => {
    if (type !== chatType) {
      setChatType(type);
      setChatHistory([{ role: 'bot', text: type === 'admin' ? t('adminInitialMessage') : t('chatInitialMessage') }]);
    }
    setIsChatOpen(true);
  };
  ...
  const handleSendMessage = () => {
    ...
    // Mock response based on chatType
    setTimeout(() => {
      const responseText = chatType === 'admin' ? t('adminResponse') : t('chatBotResponse');
      setChatHistory(prev => [...prev, { role: 'bot' as const, text: responseText }]);
    }, 1000);
  };
      ...
      case 'dashboard':
        return (
          <Dashboard 
            ...
            onOpenChat={() => handleOpenChat('admin')}
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-4">
                <div className="sticky top-24 space-y-6">
                  ...
                  {/* Removed old chatbot bubble from here as it's now global */}
                </div>
              </div>
            </form>
          </>
              ...
              <div className="bg-blue-600 p-4 text-white flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                    {chatType === 'admin' ? <User size={20} /> : <Bot size={20} />}
                  </div>
                  <div>
                    <h4 className="font-bold text-sm">{chatType === 'admin' ? t('adminMessaging') : 'Assistant Mr.dent'}</h4>
                    <div className="flex items-center gap-1.5">
                      <span className="w-2 h-2 bg-green-400 rounded-full"></span>
                      <span className="text-[10px] text-blue-100 uppercase font-bold tracking-wider">{chatType === 'admin' ? t('adminOnline') : 'En ligne'}</span>
                    </div>
                  </div>
                </div>
                <button 
      ...
      {isAuthenticated && !isChatOpen && (
        <div className="fixed bottom-8 right-8 z-50 flex flex-col items-end gap-3 pointer-events-none">
          ...
          >
            <button 
              onClick={() => handleOpenChat('bot')}
              className="w-16 h-16 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-2xl shadow-blue-500/40 hover:bg-blue-700 hover:scale-110 transition-all active:scale-95 group relative"
              title="Assistant Mr.dent"
            >
              <Bot size={28} />
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 border-2 border-white dark:border-slate-900 rounded-full animate-pulse"></span>
            </button>
          </motion.div>
        </div>
      )}
